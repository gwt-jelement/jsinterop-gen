    ...
    private void processPartials() {
        ...
        model.getDefinitions().stream()
                ...
                .forEach(primaryDefinition -> {
                    for (PartialDefinition partialDefinition : primaryDefinition.getPartialDefinitions()) {
                        if (partialDefinition instanceof PartialInterfaceDefinition
                                && primaryDefinition instanceof InterfaceDefinition) {
                            mergeInterfaces((InterfaceDefinition) primaryDefinition, (InterfaceDefinition) partialDefinition);
                        } else if (partialDefinition instanceof PartialDictionaryDefinition
                                && primaryDefinition instanceof DictionaryDefinition) {
                            mergeDictionaries((DictionaryDefinition) primaryDefinition, (DictionaryDefinition) partialDefinition);
                        } else {
                            reportTypeMismatch(primaryDefinition, (AbstractDefinition) partialDefinition);
                        }
                    }
                });
    }
